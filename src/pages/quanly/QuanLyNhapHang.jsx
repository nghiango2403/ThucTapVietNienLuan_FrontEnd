import React, { useEffect, useState } from "react";
import ReactPaginate from "react-paginate";
import { Link } from "react-router-dom";
import { toast } from "react-toastify";
import { layphieunhaphang, xoaphieunhaphang } from "../../services/Service";
import XemPhieuNhapHang from "../../components/XemPhieuNhapHang";

const QuanLyNhapHang = () => {
  const [dsNhapHang, setDsNhapHang] = useState([]);
  const [soTrang, setSoTrang] = useState(1);
  const [trangHienTai, setTrangHienTai] = useState(0);
  const [thang, setThang] = useState(() => new Date().getMonth() + 1);
  const [nam, setNam] = useState(new Date().getFullYear());
  const [hienXem, setHienXem] = useState(false);
  const [nhapHangChon, setNhapHangChon] = useState({});

  useEffect(() => {
    layDanhSachNhapHang();
  }, [trangHienTai]);

  const layDanhSachNhapHang = async () => {
    try {
      const response = await layphieunhaphang(trangHienTai + 1, thang, nam);
      setDsNhapHang(response.data.data.danhsach);
      console.log(response.data.data.sotrang);
      setSoTrang(response.data.data.sotrang);
    } catch (error) {
      console.log(error);
      toast.error("Lấy danh sách nhập hàng thất bại");
    }
  };

  const xoa = async (id) => {
    try {
      console.log(id);
      const response = await xoaphieunhaphang(id);
      layDanhSachNhapHang();
      toast.success(response.data.message);
    } catch (error) {
      console.log(error);
      toast.error("Xoá nhập hàng thất bại");
    }
  };

  const doiTrang = (event) => {
    setTrangHienTai(event.selected);
  };

  return (
    <div className="p-6 bg-gray-50 min-h-screen px-36">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">
        Quản lý nhập hàng
      </h1>
      <div className="flex items-center space-x-3 mb-4">
        <input
          type="number"
          placeholder="Tháng"
          value={thang}
          onChange={(e) => setThang(e.target.value)}
          className="w-28 px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-400 outline-none"
          min="1"
          max="12"
        />
        <input
          type="number"
          placeholder="Năm"
          value={nam}
          onChange={(e) => setNam(e.target.value)}
          className="w-32 px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-400 outline-none"
          min="2000"
          max="3000"
        />
        <button
          onClick={layDanhSachNhapHang}
          className="px-4 py-2 bg-indigo-500 text-white rounded-xl hover:bg-indigo-600"
        >
          Tìm
        </button>
        <Link
          to="/quanly/themphieunhaphang"
          className="px-4 py-2 bg-green-500 text-white rounded-xl hover:bg-green-600"
        >
          + Thêm mới
        </Link>
      </div>

      <div className="bg-white rounded-2xl shadow-md overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-4 py-3 text-sm font-semibold text-gray-600">
                STT
              </th>
              <th className="px-4 py-3 text-sm font-semibold text-gray-600">
                Thời gian nhập
              </th>
              <th className="px-4 py-3 text-sm font-semibold text-gray-600">
                Chức năng
              </th>
            </tr>
          </thead>
          <tbody>
            {dsNhapHang.length > 0 ? (
              dsNhapHang.map((item, index) => (
                <tr
                  key={item._id}
                  className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}
                >
                  <td className="px-4 py-3 text-gray-700">
                    {index + 1 + trangHienTai * 10}
                  </td>
                  <td className="px-4 py-3 text-gray-700">
                    {new Date(item.ThoiGianNhap).toLocaleString("vi-VN")}
                  </td>
                  <td className="px-4 py-3 space-x-3">
                    <button
                      className="text-blue-600 hover:underline"
                      onClick={() => {
                        setHienXem(true);
                        setNhapHangChon(item);
                      }}
                    >
                      Xem
                    </button>
                    <button
                      className="text-red-600 hover:underline"
                      onClick={() => xoa(item._id)}
                    >
                      Xoá
                    </button>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="3" className="px-4 py-6 text-center text-gray-500">
                  Không có phiếu nhập hàng nào
                </td>
              </tr>
            )}
          </tbody>
        </table>

        {hienXem && (
          <XemPhieuNhapHang
            dong={() => setHienXem(false)}
            nhapHang={nhapHangChon}
          />
        )}
      </div>

      <div className="flex justify-center mt-6">
        <ReactPaginate
          previousLabel={"← Trước"}
          nextLabel={"Sau →"}
          breakLabel={"..."}
          pageCount={soTrang}
          marginPagesDisplayed={2}
          pageRangeDisplayed={3}
          onPageChange={doiTrang}
          containerClassName="flex items-center space-x-2"
          pageClassName="px-3 py-1 rounded-md border border-gray-300 cursor-pointer"
          activeClassName="bg-indigo-500 text-white border-indigo-500"
          previousClassName="px-3 py-1 rounded-md border border-gray-300 cursor-pointer"
          nextClassName="px-3 py-1 rounded-md border border-gray-300 cursor-pointer"
          disabledClassName="opacity-50 cursor-not-allowed"
        />
      </div>
    </div>
  );
};

export default QuanLyNhapHang;
